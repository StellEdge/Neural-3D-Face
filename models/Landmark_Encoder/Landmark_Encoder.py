...
class Encoder_Landmarks(torch.nn.Module):
    def __init__(self, model_dir='models/pretrained_data/mobilefacenet_model_best.pth.tar'):
...
class Encoder_Landmarks_FULL(torch.nn.Module):
    def __init__(self, model_dir='models/pretrained_data/mobilefacenet_model_best.pth.tar'):
        super(Encoder_Landmarks_FULL, self).__init__()
        self.model = MobileFaceNet([112, 112], 136)
        checkpoint = torch.load(model_dir)
        self.model.load_state_dict(checkpoint['state_dict'])
        self.model = self.model.eval()
